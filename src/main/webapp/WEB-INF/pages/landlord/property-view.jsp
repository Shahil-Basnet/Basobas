...
<title>${fn:escapeXml(property.title)}- Property Details |
	Basobas</title>
		...
		<div class="dashboard-container">
			...
			<div class="page-header">
				...
				<div style="display: flex; gap: 0.5rem;">
					<a
						href="${pageContext.request.contextPath}/landlord/properties?action=edit&id=${property.displayId}"
						class="btn-primary"
			...
			<div
						...
						<div style="padding: 1rem;">
							<table style="width: 100%; border-collapse: collapse;">
								<tr style="border-bottom: 1px solid #e5e7eb;">
									<td style="padding: 0.75rem 0; font-weight: 600; width: 40%;">Property
										ID</td>
									<td style="padding: 0.75rem 0;">${property.displayId}</td>
								</tr>
								<tr style="border-bottom: 1px solid #e5e7eb;">
									<td style="padding: 0.75rem 0; font-weight: 600;">Property
										Type</td>
								...
								<tr style="border-bottom: 1px solid #e5e7eb;">
									...
									<td style="padding: 0.75rem 0;"><span class="status-badge"
										style="
											...
											${property.status == 'inactive' ? 'Inactive' : ''} </span></td>
								</tr>
								<tr style="border-bottom: 1px solid #e5e7eb;">
									<td style="padding: 0.75rem 0; font-weight: 600;">Created
										Date</td>
						...
						<div style="padding: 1rem;">
							<table style="width: 100%; border-collapse: collapse;">
								...
								<c:if test="${not empty property.wardNumber}">
									<tr style="border-bottom: 1px solid #e5e7eb;">
										<td style="padding: 0.75rem 0; font-weight: 600;">Ward
											Number</td>
								...
								<c:if test="${not empty property.address}">
									<tr>
										<td style="padding: 0.75rem 0; font-weight: 600;">Full
											Address</td>
					...
					<div class="dashboard-card" style="margin-bottom: 1.5rem;">
						...
						<div style="padding: 1rem;">
							<div
								...
								<div>
									<div style="font-size: 0.7rem; color: #6b7280;">Floor
										Number</div>
									<div style="font-size: 1.1rem; font-weight: 600;">${property.floorNumber != null ? property.floorNumber : 'Ground'}</div>
								</div>
								<div>
									<div style="font-size: 0.7rem; color: #6b7280;">Monthly
										Rent</div>
									<div
										style="font-size: 1.1rem; font-weight: 700; color: var(--primary);">
										रू
								...
								<div>
									<div style="font-size: 0.7rem; color: #6b7280;">Security
										Deposit</div>
								...
								<c:if
									...
									<div>
										<div style="font-size: 0.7rem; color: #6b7280;">Power
											Backup</div>
										<div style="font-size: 1.1rem; font-weight: 500;">${property.powerBackupHours}
											hours/day</div>
									</div>
								</c:if>
								<c:if test="${not empty property.waterSource}">
									<div>
										<div style="font-size: 0.7rem; color: #6b7280;">Water
											Source</div>
										<div
											style="font-size: 1.1rem; font-weight: 500; text-transform: capitalize;">${property.waterSource}</div>
									</div>
								</c:if>
								<c:if test="${not empty property.roadAccess}">
									<div>
										<div style="font-size: 0.7rem; color: #6b7280;">Road
											Access</div>
										<div
											style="font-size: 1.1rem; font-weight: 500; text-transform: capitalize;">
											${property.roadAccess == '2w' ? '2-Wheeler Only' : ''}
											${property.roadAccess == '4w' ? '4-Wheeler Only' : ''}
											${property.roadAccess == 'both' ? '2W & 4W Access' : ''}
											${property.roadAccess == 'none' ? 'No Access' : ''}</div>
									</div>
								</c:if>
							</div>
						</div>
				...
				<div>
					...
					<c:if
						test="${property.status == 'rented' and not empty currentTenant}">
						<div class="dashboard-card" style="margin-bottom: 1.5rem;">
							<div class="card-header">
							...
							<div style="padding: 1rem;">
								<div
									...
									<div
										style="width: 60px; height: 60px; background: linear-gradient(135deg, var(--primary), var(--primary-container)); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
										<span class="material-symbols-outlined"
											style="font-size: 2rem; color: white;">person</span>
									</div>
									<div>
								...
								<table style="width: 100%; border-collapse: collapse;">
									...
									<c:if test="${not empty property.currentLeaseStart}">
										<tr style="border-bottom: 1px solid #e5e7eb;">
											<td style="padding: 0.75rem 0; font-weight: 600;">Lease
												Start</td>
								...
								<div style="margin-top: 1rem; display: flex; gap: 0.5rem;">
									<a href="#" class="btn-outline"
										...
										<span class="material-symbols-outlined"
										style="font-size: 1rem;">chat</span> Message Tenant
									</a> <a
										href="${pageContext.request.contextPath}/landlord/payments?propertyId=${property.propertyId}"
										class="btn-primary"
										style="flex: 1; text-align: center; text-decoration: none;">
										<span class="material-symbols-outlined"
							...
							<c:choose>
								<c:when test="${empty requests}">
									<div style="text-align: center; padding: 2rem; color: #6b7280;">
										<span class="material-symbols-outlined"
											style="font-size: 3rem;">inbox</span>
									...
									<c:forEach var="req" items="${requests}" end="2">
										<div
											...
											<div
												...
												<div>
													<span style="font-weight: 600;">${req.tenantName}</span> <span
														style="font-size: 0.7rem; color: #6b7280; margin-left: 0.5rem;">${req.displayId}</span>
												</div>
											...
											<div style="font-size: 0.8rem; color: #6b7280;">
												Move-in: ${req.requestedMoveInDate} •
												${req.requestedLeaseDurationMonths} months</div>
											<c:if test="${not empty req.tenantMessage}">
												<div
													style="font-size: 0.75rem; background: #f9fafb; padding: 0.5rem; margin-top: 0.5rem; border-radius: 0.5rem;">
													"${fn:escapeXml(req.tenantMessage)}"</div>
											</c:if>
										</div>
									</c:forEach>
								</c:otherwise>
					...
					<div class="dashboard-card">
						<div class="card-header">
							...
							<a
								href="${pageContext.request.contextPath}/landlord/properties?action=edit&id=${property.displayId}"
								class="view-all-link">Manage Photos →</a>
						</div>
						<div style="padding: 1rem;">
							<c:choose>
								<c:when test="${empty photos}">
									<div style="text-align: center; padding: 2rem; color: #6b7280;">
										<span class="material-symbols-outlined"
											style="font-size: 3rem;">photo_camera</span>
										<p>No photos uploaded yet</p>
										<a
											href="${pageContext.request.contextPath}/landlord/properties?action=edit&id=${property.displayId}"
											class="btn-outline"
											style="margin-top: 0.5rem; display: inline-block;">Add
											Photos</a>
									</div>
								</c:when>
								<c:otherwise>
									<div
										...
										<c:forEach var="photo" items="${photos}" begin="0" end="5">
											<div
												style="position: relative; border-radius: 0.5rem; overflow: hidden; aspect-ratio: 1/1;">
												<img
													src="${pageContext.request.contextPath}/property-photo/${photo.photoUrl}"
													style="width: 100%; height: 100%; object-fit: cover;"
													onerror="this.src='${pageContext.request.contextPath}/assets/no-image.jpg'">
												<c:if test="${photo.primary}">
													<div
														style="position: absolute; top: 0.25rem; right: 0.25rem; background: var(--primary); border-radius: 50%; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center;">
														<span class="material-symbols-outlined"
															style="font-size: 0.75rem; color: white;">star</span>
													</div>
												</c:if>
											</div>
										</c:forEach>
