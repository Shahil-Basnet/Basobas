...
function updateTable(users) {
    ...
    const rows = users.map(user =>
        '<tr>' +
        '<td><input type="checkbox" class="userCheckbox" value="' + user.displayId + '"></td>' +
        '<td>' + user.displayId + '</td>' +
        '<td><div class="font-bold">' + (user.fullName || '-') + '</div></td>' +
        '<td>' + user.username + '</td>' +
        '<td>' + user.email + '</td>' +
        ...
        '<button class="action-btn action-view" onclick="viewUser(\'' + user.displayId + '\')">' +
        '<span class="material-symbols-outlined">visibility</span>' +
        '</button>' +
        '<button class="action-btn action-edit" onclick="editUser(\'' + user.displayId + '\')">' +
        '<span class="material-symbols-outlined">edit_square</span>' +
        '</button>' +
        '<button class="action-btn" style="color: #6366f1;" title="Reset Password" onclick="resetPassword(\'' + user.displayId + '\')">' +
        '<span class="material-symbols-outlined">lock_reset</span>' +
        '</button>' +
        (user.role !== 'admin' ? '<button class="action-btn action-delete" onclick="deleteUser(\'' + user.displayId + '\')">' +
...
function updateBulkActions() {
    ...
    document.querySelector('.bulk-actions').style.display = checked.length > 0 ? 'block' : 'none';
}
...
function resetPassword(displayId) {
    if (confirm('Are you sure you want to reset the password for this user? It will be set to a default value.')) {
...
// Replace alerts in your functions:
// Instead of: alert('User deleted successfully!');
// Use: showToast('User deleted successfully!', 'success');

// Instead of: alert('Error deleting user');
// Use: showToast('Error deleting user', 'error');
